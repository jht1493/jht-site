...
const config_fb = {
  ...
  n_per_row: 4,
  n_per_page: 32,
  head: '# [John Henry Thompson](../README.md)',
  title1: 'post Facebook.com',
  title2: 'Migrated from https://www.facebook.com/johnhenrythompson'
};

const config_instag = {
  md_path: '../../jht-instag-md',
  ...
  n_per_page: 32,
  head: '# [John Henry Thompson](../README.md)',
  title1: 'post instagram.com',
  title2: 'Migrated from https://www.instagram.com/johnhenrythompson1493'
};

const config = config_instag;
...
function write_pages_mds(pages_mds, pages_mds_date, from_date, to_date, nposts_n) {
  ...
  const lns = [];
  lns.push(config.head);
  lns.push(config.title1);
  lns.push('');
  lns.push(config.title2);
  lns.push('');
...
function write_pagemd(npost, thumb_posts, index) {
  ...
  const mds = [];
  mds.push(config.head);
  mds.push(config.title1);
  mds.push('');
  let nav = [];
  if (index > 0) {
